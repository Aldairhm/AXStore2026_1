...
class Salida
{
    ...
    // Detecta si ya existe una salida para la misma variante
    // y misma cantidad en los últimos 10 segundos
    // ============================================================
    public function existeSalidaReciente(int $idVariante, int $cantidad): bool
    {
        $sql = "SELECT COUNT(*) FROM salida 
    ...
    public function obtenerTodasLasSalidas(): array
    {
        ...
        $sql = "SELECT 
                    s.id, s.id_variante, s.id_usuario, s.cantidad,
    ...
    {
        $sql  = "SELECT COUNT(*) FROM salida WHERE DATE(fecha_salida) = CURDATE()";
        $stmt = $this->conexion->prepare($sql);
        $stmt->execute();
        return (int)$stmt->fetchColumn();
    }
    ...
    public function puedeDevolver(int $id): array
    {
        ...
        if (!$salida) {
            return ['puede' => false, 'motivo' => 'Salida no encontrada'];
        }

        if ($salida['estado'] === 'Cancelado') {
            return ['puede' => false, 'motivo' => 'Esta salida ya fue cancelada anteriormente'];
        }

        $diasRestantes = null;
        if ($salida['fecha_entrega']) {
            $diasRestantes = (new DateTime())->diff(new DateTime($salida['fecha_entrega']))->days;
        }

        return ['puede' => true, 'motivo' => 'Devolución permitida', 'dias_restantes' => $diasRestantes];
    }
    ...
    public function procesarDevolucion(int $id, string $motivo = '', ?int $cantidadADevolver = null): array
    {
        ...
        if (!$salida)                          return ['exito' => false, 'mensaje' => 'Salida no encontrada'];
        if ($salida['estado'] === 'Cancelado') return ['exito' => false, 'mensaje' => 'Esta salida ya fue cancelada'];
        ...
        try {
            ...
            if ($esTotal) {
                ...
                $sql  = "UPDATE salida SET cantidad=:cantidad, subtotal=:subtotal, total=:total, observaciones=:motivo WHERE id=:id";
                $stmt = $this->conexion->prepare($sql);
                if (!$stmt->execute([
                    ':cantidad' => $nuevaCantidad,
                    ':subtotal' => $nuevoSubtotal,
                    ':total'    => $nuevoTotal,
                    ':motivo'   => $motivo ?: $salida['observaciones'],
                    ':id'       => $id
                ])) {
            ...
            $mensaje = $esTotal
                ...
                : "Devolución parcial: se devolvieron {$cantidadADevolver} unidades. La salida continúa con " . ($cantidadOriginal - $cantidadADevolver) . " unidades.";
    ...
    public function limpiarSalidasVencidas(): void
    {
        try {
            $this->conexion->exec(
                "DELETE FROM salida WHERE estado = 'Cancelado' AND fecha_cancelacion <= DATE_SUB(NOW(), INTERVAL 3 DAY)"
            );
        } catch (Exception $e) {
