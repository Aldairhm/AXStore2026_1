...
$(document).ready(function () {
  ...
  function limpiarErroresFormulario() {
    $("#nombre_real, #username, #rol, #password, #estado").removeClass(
      "is-invalid",
    );
  }
  ...
  const tabla = $("#tablaUsuarios").DataTable({
    ...
    columns: [
      {
        data: "nombre_real",
        createdCell: (td) => $(td).attr("data-label", "Nombre"),
      },
      {
        data: "username",
        createdCell: (td) => $(td).attr("data-label", "Usuario"),
      },
      {
        data: "rol",
        className: "text-center",
        createdCell: (td) => $(td).attr("data-label", "Rol"),
      ...
      },
      {
        data: "estado",
        className: "text-center",
        createdCell: (td) => $(td).attr("data-label", "Estado"),
      ...
      },
      {
        ...
        searchable: false,
        className: "text-center",
        createdCell: (td) => $(td).attr("data-label", "Acciones"),
        render: function (row) {
          // Aplicamos el estándar: btn-kebab-luxury y quitamos clases de color del icono
          return `
        <div class="dropdown">
            <button class="btn btn-kebab-luxury shadow-none border-0" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                <i class="fas fa-ellipsis-v"></i>
            </button>
            <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2 animate__animated animate__fadeIn">
                <li>
                    <a class="dropdown-item py-2 btn-editar" data-id="${row.id}" href="#">
                        <i class="fas fa-edit me-2 text-warning"></i> Editar Usuario
                    </a>
                </li>
                <li>
                    <a class="dropdown-item py-2 btn-eliminar text-danger" data-id="${row.id}" href="#">
  ...
  $(document).on("click", ".btn-abrir-modal-usuario", function () {
    // Añade esta clase a tu botón de "Nuevo Usuario"
